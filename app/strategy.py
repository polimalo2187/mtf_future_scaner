...
def mtf_strategy(
    ...
    """
    Retorna una señal con SCORE proporcional y realista,
    diferenciando señales Oro / Plata / Bronce.
    """
    ...
    if entry_confirmation(df_5m, direction):
        last = df_5m.iloc[-1]
        # Score proporcional basado en cercanía al EMA (0–30)
        distance = abs(last["close"] - last["ema_fast"])
        proximity_score = max(0, 30 - distance * 1000)  # Escala realista
