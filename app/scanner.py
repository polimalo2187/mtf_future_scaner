        ...
        try:
            ...
            for symbol in symbols:
                try:
                    ...
                    if result:
                        logger.info(
                            f"🎯 Setup válido detectado en {symbol} | score={result.get('score')}"
                        )
                        result["symbol"] = symbol
                        candidates.append(result)
                    else:
                        logger.debug(f"❌ {symbol} descartado por estrategia")
            ...
            logger.info(f"📌 Total setups válidos: {len(candidates)}")
            ...
            logger.info("🏆 Ranking global generado (TOP 3)")
            ...
            for idx, signal in enumerate(top_3):
                ...
                logger.info(
                    f"✅ Señal creada | {medal} {symbol} {direction} | score={score}"
                )
            ...
            logger.info("⏸️ Ciclo finalizado, esperando siguiente ejecución")
            await asyncio.sleep(SCAN_INTERVAL_SECONDS)
