...
async def handle_exchange_text_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        exchange_name = update.message.text.strip().lower()  # Guardamos siempre en minúsculas
        users_col = users_collection()
        user_id = update.effective_user.id
        ...
        await update.message.reply_text(
            f"✅ Exchange confirmado: {exchange_name.capitalize()}\nMenú principal:",
            reply_markup=main_menu(),
        )
...
async def handle_view_signals(query, user, admin, users_col):
    try:
        ...
        for sig in base_signals:
            supported_exchanges = [e.lower() for e in sig.get("supported_exchanges", ["binance"])]
            if admin or user_exchange in supported_exchanges:
                selected_signal = sig
                break
        ...
        user_signal["exchange"] = user_exchange.capitalize()
