...
# ======================================================
# FORMATEAR SEÑAL USUARIO (FORMATO EXACTO)
# ======================================================

def format_user_signal(user_signal: Dict) -> str:
    tz = pytz.timezone(USER_TIMEZONE)
    start_time = user_signal["created_at"].astimezone(tz)
    end_time = user_signal["telegram_valid_until"].astimezone(tz)

    plan_name = user_signal.get("visibility", PLAN_FREE).upper()
    ...
    text = (
        ...
        f"━━━━━━━━━━━━━━━━━━\n"
        f"CONSERVADOR\n"
        f"SL: {user_signal['profiles']['conservador']['stop_loss']}\n"
    )
    for idx, tp in enumerate(user_signal['profiles']['conservador']['take_profits'], 1):
        text += f"TP{idx}: {tp}\n"
    text += f"Apalancamiento: {user_signal['leverage_profiles']['conservador']}\n\n"

    text += f"━━━━━━━━━━━━━━━━━━\n"
    text += f"MODERADO\n"
    text += f"SL: {user_signal['profiles']['moderado']['stop_loss']}\n"
    for idx, tp in enumerate(user_signal['profiles']['moderado']['take_profits'], 1):
        text += f"TP{idx}: {tp}\n"
    text += f"Apalancamiento: {user_signal['leverage_profiles']['moderado']}\n\n"

    text += f"━━━━━━━━━━━━━━━━━━\n"
    text += f"AGRESIVO\n"
    text += f"SL: {user_signal['profiles']['agresivo']['stop_loss']}\n"
    for idx, tp in enumerate(user_signal['profiles']['agresivo']['take_profits'], 1):
        text += f"TP{idx}: {tp}\n"
    text += f"Apalancamiento: {user_signal['leverage_profiles']['agresivo']}\n\n"

    text += f"⏳ Activa: {start_time.strftime('%H:%M')} → {end_time.strftime('%H:%M')}\n"
