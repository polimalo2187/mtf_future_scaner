...
async def handle_my_account(query, user, admin=False):
    # Administradores siempre aparecen como PREMIUM
    plan = PLAN_PREMIUM if admin else user.get("plan", PLAN_FREE)
    ...
    # Si es administrador, añadir panel
    if admin:
        message += "\n👑 PANEL ADMINISTRADOR\n"
        keyboard = [
            [InlineKeyboardButton("➕ Activar plan", callback_data="admin_activate_plan")],
...
async def handle_admin_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if context.user_data.get("awaiting_plan_selection"):
        plan_choice = update.message.text.strip().lower()
        target_user_id = context.user_data.get("target_user_id")
        context.user_data["awaiting_plan_selection"] = False
        context.user_data["target_user_id"] = None

        if plan_choice == "plus":
            if activate_plus(target_user_id):
                await update.message.reply_text("✅ Plan PLUS activado.")
            else:
                await update.message.reply_text("❌ No se pudo activar el plan PLUS.")
        elif plan_choice == "premium":
            if activate_premium(target_user_id):
                await update.message.reply_text("✅ Plan PREMIUM activado.")
            else:
                await update.message.reply_text("❌ No se pudo activar el plan PREMIUM.")
        else:
            await update.message.reply_text("❌ Opción inválida. Envía 'PLUS' o 'PREMIUM'.")
        return
    ...
    context.user_data["awaiting_plan_selection"] = True
    context.user_data["target_user_id"] = target_user_id
    await update.message.reply_text("📌 Envía 'PLUS' o 'PREMIUM' para activar el plan correspondiente.")
...
def get_handlers():
    return [
        CallbackQueryHandler(handle_menu, pattern="^(view_signals|plans|my_account|referrals|support|admin_panel|admin_activate_plan|back_menu)$"),
