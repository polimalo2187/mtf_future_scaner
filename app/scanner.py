...
async def scan_market_async(bot: Bot):
    ...
    while True:
        try:
            # 🔒 BLOQUEO TOTAL MIENTRAS HAYA SEÑAL VIGENTE
            while not can_create_new_signal():
                now = datetime.utcnow()
                ...
                if last_signal:
                    remaining = (last_signal["telegram_valid_until"] - now).total_seconds()
                    remaining = max(1, int(remaining))
                    logger.info(f"⏳ Señal vigente en Telegram, escaneo detenido {remaining}s restantes")
                    await asyncio.sleep(remaining)
                else:
                    break  # Ninguna señal activa, continuar
            ...
            # 🔹 CREAR LAS 3 SEÑALES SIMULTÁNEAS PARA BLOQUEO POSTERIOR
            created_signals = []
            for idx, signal in enumerate(selected):
                visibility = PLAN_ORDER[idx] if idx < len(PLAN_ORDER) else PLAN_FREE
                ...
                if base_signal:
                    created_signals.append(base_signal)
            ...
            # ⏳ BLOQUEAR ESCANEO HASTA QUE LAS 3 SEÑALES HAYAN VENCIDO 15 MIN
            if created_signals:
                latest_expiry = max(sig["telegram_valid_until"] for sig in created_signals)
                now = datetime.utcnow()
                wait_seconds = max(1, int((latest_expiry - now).total_seconds()))
                logger.info(f"⏳ Esperando a que caduquen las señales creadas ({wait_seconds}s)")
