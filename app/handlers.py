...
# ======================================================
# HANDLER PARA ESTADÍSTICAS DE ADMINISTRADOR
# ======================================================
...
# ======================================================
# HANDLER REFERRALS - CORREGIDO DEFINITIVAMENTE
# ======================================================

async def handle_referrals(query, user):
    """Muestra la información de referidos del usuario con enlace completo."""
    try:
        # ✅ VERIFICACIÓN 1: Usuario válido
        if not user:
            logger.error("⚠️ handle_referrals: Objeto 'user' es None o vacío")
            await query.edit_message_text(
                "❌ Datos de usuario no disponibles. Usa /start nuevamente.",
                reply_markup=back_to_menu(),
            )
            return

        # ✅ VERIFICACIÓN 2: user_id presente y válido
        user_id = user.get("user_id")
        if not user_id:
            logger.error(f"⚠️ handle_referrals: user_id no encontrado en user: {user}")
            await query.edit_message_text(
                "❌ ID de usuario no disponible. Usa /start nuevamente.",
                reply_markup=back_to_menu(),
            )
            return

        logger.info(f"🔍 Buscando estadísticas de referidos para user_id: {user_id}")

        # ✅ VERIFICACIÓN 3: Obtener estadísticas con manejo de errores
        try:
            stats = get_user_referral_stats(user_id)
        except Exception as db_error:
            logger.error(f"❌ Error en get_user_referral_stats para {user_id}: {db_error}", exc_info=True)
            await query.edit_message_text(
        ...
        # ✅ VERIFICACIÓN CORREGIDA: Ahora verifica si stats es None (error) o si no tiene referidos
        if stats is None:
            logger.error(f"❌ get_user_referral_stats retornó None para user_id: {user_id}")
            await query.edit_message_text(
                "❌ Error crítico en el sistema de referidos. Contacta al soporte.",
                reply_markup=back_to_menu(),
            )
            return
        ...
        if total_referred == 0:
            # Usuario sin referidos - mostrar mensaje amigable con su código
            bot_username = query.bot.username
            ref_code = stats.get('ref_code', f"ref_{user_id}")
            ...
            message = (
                ...
                f"`{ref_link_display}`\n\n"
                "📋 *Código para usar con /start:*\n"
                f"`{ref_code}`\n\n"
                "📢 *Cómo comenzar:*\n"
                "1. Comparte tu enlace con amigos\n"
                "2. Ellos hacen clic y se unen al bot\n"
                "3. Cuando activen un plan, tú ganas\n\n"
                "✨ *Recompensas disponibles:*\n"
                "• 5 referidos PLUS = Plan PLUS GRATIS\n"
                "• 5 referidos PREMIUM = Plan PREMIUM GRATIS"
            )
        ...
        # Si tiene referidos, mostrar estadísticas completas
        bot_username = query.bot.username
        ref_code = stats.get('ref_code', f"ref_{user_id}")
        ...
        message += f"• Total referidos: `{stats.get('total_referred', 0)}`\n"
        ...
        message += f"• PLUS válidos: `{stats.get('current_plus', 0)}/5`\n"
        message += f"• PREMIUM válidos: `{stats.get('current_premium', 0)}/5`\n\n"
        ...
        logger.info(f"✅ Estadísticas de referidos mostradas para user {user_id}")
        
    except KeyError as e:
        logger.error(f"❌ Falta clave en datos del usuario: {e}. Usuario: {user}", exc_info=True)
        await query.edit_message_text(
            "❌ Error en datos del usuario. Por favor, usa /start nuevamente.",
            reply_markup=main_menu(),
        )
    except Exception as e:
        logger.error(f"❌ Error en handle_referrals para user {user.get('user_id', 'unknown')}: {e}", exc_info=True)
        await query.edit_message_text(
            "❌ Error al cargar información de referidos. Por favor, intenta nuevamente.",
            reply_markup=back_to_menu(),
          )

  # ======================================================
# CONTINUACIÓN - app/handlers.py
# ======================================================
...
async def handle_plans(query):
    ...
    try:
        ...
        logger.error(f"❌ Error en handle_plans: {e}", exc_info=True)
        await query.edit_message_text(
...
async def handle_my_account(query, user):
    ...
    try:
        ...
        logger.error(f"❌ Error en handle_my_account para user {user['user_id']}: {e}", exc_info=True)
        await query.edit_message_text(
...
async def handle_support(query):
    ...
    try:
        ...
        logger.error(f"❌ Error en handle_support: {e}", exc_info=True)
        await query.edit_message_text(
...
# ======================================================
# REGISTRO DE HANDLERS (COMPLETO Y FUNCIONAL)
# ======================================================
