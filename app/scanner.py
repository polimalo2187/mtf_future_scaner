...
async def scan_market_async(bot: Bot):
    ...
    while True:
        try:
            ...
            # 🔹 ENVÍO DE LAS TRES SEÑALES SIMULTÁNEAMENTE
            for idx, signal in enumerate(selected):
                ...
                # Solo bloquear después de enviar la última señal
                is_last_signal = (idx == len(selected) - 1)
                ...
                if base_signal:
                    ...
                    if is_last_signal:
                        # Activar bloqueo de nueva señal por 15 minutos
                        now = datetime.utcnow()
                        base_signal["telegram_valid_until"] = now + timedelta(minutes=15)
