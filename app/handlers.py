...
from app.plans import PLAN_FREE, PLAN_PLUS, PLAN_PREMIUM, activate_premium, plan_status
...
async def handle_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if action == "user_report":
            await handle_user_report(query, user)
            return
...
# ======================================================
# HANDLER REPORTE PERSONAL (NUEVO)
# ======================================================

async def handle_user_report(query, user):
    try:
        user_id = user["user_id"]
        username = user.get("username", "Sin nombre")
        plan_info = plan_status(user)
        stats = get_user_referral_stats(user_id)

        message = f"📊 TU REPORTE PERSONAL – BOT\n\n"
        message += f"👤 Usuario: @{username}\n🆔 ID: {user_id}\n\n"

        message += f"🚀 ESTADO DE TU CUENTA\n"
        message += f"• Plan activo: {plan_info['plan'].upper()}\n"
        expires = plan_info['expires'].strftime("%d/%m/%Y") if plan_info['expires'] else "N/A"
        message += f"• Vence: {expires}\n"
        message += f"• Estado: {'✅ Activo' if plan_info['status']=='active' else '⏳ ' + plan_info['status'].capitalize()}\n\n"

        message += f"👥 SISTEMA DE REFERIDOS\n"
        message += f"🔗 Tu link personal:\nhttps://t.me/MTFSignsls_bot?start=ref_{stats['ref_code']}\n\n"
        message += f"📊 Estadísticas\n"
        message += f"• Referidos totales: {stats['total_referred']}\n"
        message += f"• Referidos PLUS: {stats['plus_referred']}\n"
        message += f"• Referidos PREMIUM: {stats['premium_referred']}\n\n"
        message += f"🎯 Contadores activos\n"
        message += f"• PLUS válidos: {stats['current_plus']} / 5\n"
        message += f"• PREMIUM válidos: {stats['current_premium']} / 5\n\n"

        message += "🎁 Recompensas pendientes:\n"
        if stats['pending_rewards']:
            for r in stats['pending_rewards']:
                message += f"• {r}\n"
        else:
            message += "📝 No tienes recompensas pendientes\n"
        ...
        logger.error(f"Error en handle_user_report: {e}", exc_info=True)
        await query.edit_message_text(
...
def get_handlers():
    return [
        CallbackQueryHandler(handle_menu, pattern="^(view_signals|plans|my_account|referrals|support|admin_panel|admin_activate_plan|back_menu|user_report)$"),
