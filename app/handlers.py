...
# ======================================================
# HANDLER VIEW SIGNALS (CORREGIDO PARA EXCHANGE)
# ======================================================
...
async def handle_view_signals(query, user, admin, users_col):
    try:
        ...
        base_signals = get_latest_base_signal_for_plan(user)
        if not base_signals:
            await query.edit_message_text(
        ...
        for sig in base_signals:
            supported_exchanges = sig.get("supported_exchanges", ["binance"])
            if admin or user_exchange.lower() in [e.lower() for e in supported_exchanges]:
                selected_signal = sig
                break
        ...
        user_signal = generate_user_signal(selected_signal, user)
        # Garantizar que exchange se muestre correctamente
...
def get_handlers():
    return [
        CallbackQueryHandler(
            handle_menu,
            pattern="^(view_signals|plans|my_account|referrals|support|admin_panel|admin_activate_plan|choose_plus_plan|choose_premium_plan|register_exchange|back_menu)$"
        ),
        CallbackQueryHandler(
            handle_copy_ref_code,
            pattern="^copy_ref_code$"
        ),
        MessageHandler(
