...
from app.models import is_admin  # 🔹 IMPORT CRÍTICO PARA ADMIN
...
def get_latest_base_signal_for_user(user_id: int, user_plan: str) -> Optional[List[Dict]]:
    """
    Devuelve las señales correspondientes para el usuario o admin.
    - Admin recibe todas las señales: FREE + PLUS + PREMIUM
    - Usuario normal recibe solo señales de su plan
    """
...
def format_user_signal(signal: Dict) -> str:
    ...
    start_cuba = signal["created_at"].replace(tzinfo=ZoneInfo("UTC")).astimezone(cuba_tz)
    end_cuba = signal["valid_until"].replace(tzinfo=ZoneInfo("UTC")).astimezone(cuba_tz)
