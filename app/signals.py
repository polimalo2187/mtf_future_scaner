...
def can_create_new_signal() -> bool:
    """
    Retorna True solo si no hay ninguna señal activa en Telegram.
    Bloquea creación/envío mientras exista alguna con telegram_valid_until > now.
    """
    now = datetime.utcnow()
...
def create_base_signal(
    ...
    # NO crear nueva señal si ya hay una vigente en Telegram
    ...
    # Establecer vigencia de Telegram según horario de Cuba
    cuba_open_hour = 9
    cuba_close_hour = 21
    today_open = now.replace(hour=cuba_open_hour, minute=0, second=0, microsecond=0)
    today_close = now.replace(hour=cuba_close_hour, minute=0, second=0, microsecond=0)
    if now < today_open:
        telegram_valid_until = today_open + timedelta(minutes=15)
    elif now > today_close:
        telegram_valid_until = today_close + timedelta(minutes=15)
    else:
