...
# ======================================================
# BLOQUEO TELEGRAM (15 MIN) — UNIFICADO
# ======================================================

def telegram_signal_blocked(visibility: str) -> bool:
    now = datetime.utcnow()
    last_signal = signals_collection().find_one(
        {"visibility": visibility, "telegram_valid_until": {"$gt": now}},
        sort=[("created_at", -1)]
    )
    blocked = last_signal is not None
    if blocked:
        logger.info(f"⏸️ Bloqueo activo en Telegram para {visibility}")
    return blocked

# ======================================================
# DETERMINACIÓN DE PLAN POR SCORE
# ======================================================

def get_signal_visibility_by_score(score: float) -> str:
    if score >= 90:
        return PLAN_PREMIUM
    elif 80 <= score < 90:
        return PLAN_PLUS
        ...
        return None  # No enviar señal si no llega al mínimo
    ...
    while True:
        try:
            ...
            for signal in candidates:
                ...
                if not visibility:
                    logger.info(f"⚠️ Señal {signal['symbol']} con score {score} no alcanza mínimo para ningún plan")
                    continue
                ...
                if recent_duplicate_exists(symbol, direction, visibility):
                    logger.info(f"♻️ Señal duplicada ignorada: {symbol} {direction} ({visibility})")
                    continue
                ...
                logger.info(f"📌 Creando base de señal: {symbol} {direction} ({visibility})")
                ...
                if base_signal is None:
                    logger.info(f"⚠️ Base de señal no creada para {symbol} ({visibility})")
                    continue

                logger.info(f"📤 Enviando push a Telegram para {symbol} ({visibility})")
