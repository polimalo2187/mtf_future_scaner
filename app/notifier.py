...
# ======================================================
# ALERTA PUSH (USUARIOS + ADMIN)  ✅ OPCIÓN C
# ======================================================
...
async def notify_new_signal_alert(
    ...
    """
    USUARIOS:
    - Reciben PUSH SOLO si su plan coincide EXACTAMENTE con signal_visibility
    - Mensaje auto-borrado para mantener el chat limpio

    ADMINISTRADORES:
    - Reciben mensaje informativo (tipo, par, dirección y hora Cuba)
    """
    ...
    for user_id in user_ids:
        try:
            ...
            asyncio.create_task(_auto_delete(bot, user_id, message.message_id))
    ...
    if created_at:
        try:
            hora_cuba = (
                created_at.replace(tzinfo=ZoneInfo("UTC"))
                .astimezone(cuba_tz)
                .strftime("%H:%M")
            )
        except Exception:
    ...
    admin_text = (
        "👑 *ALERTA ADMIN*\n\n"
        f"Tipo: *{signal_visibility}*\n"
        f"Par: {symbol or 'N/D'}\n"
        f"Dirección: {direction or 'N/D'}\n"
        f"Hora: {hora_cuba} (Cuba 🇨🇺)\n\n"
        "Señal creada correctamente."
    )
