...
# ======================================================
# BLOQUEO GLOBAL POR SEÑAL FREE
# ======================================================

def can_create_new_signal() -> bool:
    """
    SOLO bloquea cuando existe una señal FREE activa en Telegram.
    Premium y Plus se permiten dentro del mismo ciclo.
    """
    now = datetime.now(LOCAL_TZ)
    active_free = signals_collection().find_one(
        {
...
def create_base_signal(
    ...
    if visibility == PLAN_FREE and not can_create_new_signal():
        logger.info("⛔ Señal FREE activa en Telegram, bloqueo total")
        return None
    ...
    now = datetime.now(LOCAL_TZ)
    telegram_until = now + timedelta(minutes=15)
    ...
    signal.update({
        ...
        "telegram_valid_until": telegram_until,
        "evaluated": False,
        "entry_zone": {
            "low": str(zone_low),
            "high": str(zone_high),
    ...
    logger.info(f"📤 Señal enviada a Telegram ({visibility}): {signal['_id']}")
...
def format_user_signal(signal: Dict) -> str:
    ...
    timer_text = (
        f"⏳ Vigencia Telegram: {remaining_minutes:02d}:{remaining_seconds:02d}\n\n"
    ...
    text = (
        ...
        f"⏱️ Tiempo estimado: {est['min']} – {est['max']} minutos\n\n"
        + timer_text +
