...
# ⏱️ Frecuencia de vigilancia (NO de envío)
    ...
    while True:
        try:
            ...
            for idx, signal in enumerate(top_3):
                ...
                entry_price = float(signal.get("entry_price") or signal.get("entry") or 0)
                score = signal["score"]
                ...
                # Adaptación correcta a notify_new_signal_alert
                try:
                    await notify_new_signal_alert(bot, base_signal)
                except Exception as e:
