...
def calculate_signal_validity(timeframes: List[str]) -> int:
    """
    La vigencia se basa SIEMPRE en el timeframe MAYOR
    """
    minutes = [
...
# ======================================================
# ZONA DE ENTRADA (RETROCOMPATIBLE)
# ======================================================

def calculate_entry_zone(entry_price: float, pct: float = 0.0015):
    """
    Zona de entrada dinámica ±0.15%
    """
    low = round(entry_price * (1 - pct), 4)
    high = round(entry_price * (1 + pct), 4)
    return low, high
...
def create_base_signal(
    ...
    signal = new_signal(
        symbol=symbol,
        direction=direction,
        entry=str(entry_price),  # se mantiene para compatibilidad
    ...
    now = datetime.utcnow()
    validity_minutes = calculate_signal_validity(timeframes)

    signal["margin_mode"] = MARGIN_MODE
    signal["created_at"] = now
    signal["valid_until"] = now + timedelta(minutes=validity_minutes)
...
# ======================================================
# FORMATO FINAL DE SEÑAL (MENSAJE)
# ======================================================
...
def format_user_signal(signal: Dict) -> str:
    ...
    if signal.get("entry_zone"):
        text += (
            f"Zona de entrada: "
            f"{signal['entry_zone']['low']} – {signal['entry_zone']['high']}\n\n"
        )
    else:
        text += f"Entrada base: {signal['entry']}\n\n"
