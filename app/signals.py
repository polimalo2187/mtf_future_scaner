...
async def scan_market_async(bot: Bot):
    ...
    while True:
        try:
            # Bloqueo total si hay señal Free vigente
            if not can_create_new_signal():
                logger.info("⏳ Señal FREE activa, esperando desbloqueo...")
            ...
            # Ordenar por score descendente y tomar las 3 mejores
            candidates.sort(key=lambda x: x["score"], reverse=True)
            selected = candidates[:3]

            # Enviar señales en orden: Premium → Plus → Free
            for idx, signal in enumerate(selected):
                ...
                # Solo bloquea después de la señal Free (última del ciclo)
                is_last_signal = (visibility == PLAN_FREE)
