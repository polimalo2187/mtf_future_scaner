...
async def handle_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if not user:
            ...
            # Usuario existente: solo asignar referred_by si no existe
        ...
        if action in ["choose_plus_plan", "choose_premium_plan"]:
            ...
            if target_user_id:
                ...
                if success:
                    ...
                    await query.edit_message_text(
                        f"✅ Plan {plan_name} activado correctamente."
                    )
                else:
                    await query.edit_message_text(
                        f"❌ No se pudo activar el plan {plan_name}."
                    )
...
async def handle_referrals(query, user):
    try:
        ...
        message += (
            ...
            "3. Activan un plan\n\n"
            "📌 REGLAS:\n"
            "• FREE: 5 PLUS = Plan PLUS gratis\n"
            "• FREE: 5 PREMIUM = Plan PREMIUM gratis\n"
            "• PLUS: 5 PLUS = Extender plan\n"
            "• PLUS: 5 PREMIUM = Subir a PREMIUM\n"
            "• PREMIUM: 5 PREMIUM = Extender plan\n"
            "• PREMIUM: 10 PLUS = Extender plan\n"
        )
...
async def handle_copy_ref_code(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await query.edit_message_text(
            text=f"📋 Tu enlace de referido es:\n\n{ref_link}\n\nCópialo y compártelo.",
            reply_markup=InlineKeyboardMarkup([
...
async def handle_view_signals(query, user, admin, users_col):
    try:
        ...
        # ======================================================
        # ÚNICA MODIFICACIÓN: MANEJO DE SEÑAL INCOMPATIBLE
        # ======================================================
        user_signal = generate_user_signal(base_signal, user_id)

        if not user_signal:
            await query.edit_message_text(
                "⚠️ Señal no disponible para tu exchange.",
                reply_markup=back_to_menu(),
            )
            return
        # ======================================================
...
def get_handlers():
    return [
        CallbackQueryHandler(
            handle_menu,
            pattern=(
                "^(view_signals|plans|my_account|referrals|support|admin_panel|"
                "admin_activate_plan|register_exchange|back_menu|"
                "choose_plus_plan|choose_premium_plan)$"
            ),
        ),
