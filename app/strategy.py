...
def mtf_strategy(
    ...
    para diferenciar señales Oro / Plata / Bronce.
    """
    ...
    if entry_confirmation(df_5m, direction):
        # El score de entrada se escala según proximidad al EMA
        last = df_5m.iloc[-1]
        ema_diff = abs(last["close"] - last["ema_fast"])
        proximity_score = max(0, 30 - ema_diff*1000)  # Mientras más cerca del EMA, más score
    ...
    # =====================
    # Normalización final (garantiza 0–100)
    # =====================
