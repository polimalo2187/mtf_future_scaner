...
# ======================================================
# FORMATEAR SEÑAL PARA USUARIO (NUEVA FUNCIÓN)
# ======================================================

def format_user_signal(user_signal: Dict) -> str:
    """
    Devuelve un string legible de la señal para notificaciones.
    """
    return (
        f"{user_signal['symbol']} {user_signal['direction']} | "
        f"Entry: {user_signal['entry_price']} | "
        f"Stop: {user_signal['profiles']['moderado']['stop_loss']} | "
        f"TPs: {user_signal['profiles']['moderado']['take_profits']} | "
        f"Score: {user_signal.get('score', 0)}"
          )
