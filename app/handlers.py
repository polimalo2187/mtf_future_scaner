        ...
        if admin:
            await query.edit_message_text(
                ...
                reply_markup=InlineKeyboardMarkup([
                    ...
                    [InlineKeyboardButton("🟢 Activar cuenta", callback_data="admin_activate_plan")],
                    [InlineKeyboardButton("⬅️ Volver", callback_data="back_menu")],
                ]),
            )
        else:
...
async def handle_admin_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Activación lógica: no se activa automáticamente, se indica contactar admin
    await update.message.reply_text(
        f"🟢 Solicitud de activación para el usuario {user_id}.\n\n"
        f"Para activar el plan (PLUS o PREMIUM), contacta a un administrador:\n"
        f"{_format_whatsapp_contacts()}"
    )
