...
from app.plans import PLAN_FREE, PLAN_PLUS, PLAN_PREMIUM, activate_plan
...
async def handle_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "view_signals":
        ...
        if admin:
            plans = [PLAN_PREMIUM, PLAN_PLUS, PLAN_FREE]
...
async def handle_admin_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Preguntamos si el administrador desea activar el plan Plus o Premium
    await update.message.reply_text(
        "¿Qué plan deseas activar?\n\n"
        "1️⃣ Para activar el plan PLUS, escribe 'PLUS'.\n"
        "2️⃣ Para activar el plan PREMIUM, escribe 'PREMIUM'."
    )
    context.user_data["awaiting_plan"] = user_id  # Guardamos
