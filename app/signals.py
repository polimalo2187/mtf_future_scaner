...
def generate_user_signal(base_signal: Dict, user_id: int, user_plan: Optional[str] = None) -> Dict:
    """
    Genera señal personalizada asegurando visibilidad según el plan del usuario.
    """
    ...
    user_entry = vary(float(base_signal["entry_price"]), 0.0005)
...
# ======================================================
# FORMATEAR SEÑAL USUARIO (para Telegram)
# ======================================================

def format_user_signal(user_signal: Dict) -> str:
    symbol = user_signal.get("symbol")
    direction = user_signal.get("direction")
    entry = user_signal.get("entry_price")
    stop_loss = user_signal.get("profiles", {}).get("conservador", {}).get("stop_loss", "")
    take_profits = user_signal.get("profiles", {}).get("conservador", {}).get("take_profits", [])

    text = f"📊 Señal {symbol}\n"
    text += f"Dirección: {direction}\n"
    text += f"Entrada: {entry}\n"
    text += f"Stop Loss: {stop_loss}\n"
    text += f"Take Profits: {take_profits}\n"

    return text

# ======================================================
# OBTENER ÚLTIMA SEÑAL PARA PLAN (MODIFICADA PARA USER_SIGNALS)
# ======================================================

def get_latest_base_signal_for_plan(user_id: int, user_plan: Optional[str] = None):
    """
    Devuelve las señales personalizadas para el usuario según su plan.
    """
    if user_plan is None:
        user_plan = PLAN_FREE

    # Si es admin, da acceso a PREMIUM
    visibility = PLAN_PREMIUM if is_admin(user_id) else user_plan

    # Buscar señales activas del usuario en user_signals_collection
    signals = list(user_signals_collection().find(
        {
            "user_id": user_id,
            "visibility": visibility,
