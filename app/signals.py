...
# ======================================================
# EVALUAR SEÑALES EXPIRADAS (GANADA / PERDIDA)
# ======================================================

def evaluate_expired_signals():
    """
    Evalúa señales base expiradas y registra su resultado.
    """
    now = datetime.utcnow()
    expired = signals_collection().find(
        ...
        try:
            ...
            if signal["direction"] == "LONG":
                if current_price <= stop_loss:
                    result = "lost"
                elif any(current_price >= tp for tp in tps):
                    result = "won"
            else:
                if current_price >= stop_loss:
                    result = "lost"
