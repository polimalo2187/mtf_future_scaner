...
# =========================
# ESTADÍSTICAS DE REFERIDOS PARA USUARIO - CORREGIDO
# =========================

def get_user_referral_stats(user_id: int) -> Optional[Dict]:
    """
    Obtiene las estadísticas de referidos de un usuario.
    Retorna un diccionario con la información o None si hay error crítico.
    """
    try:
        users_col = users_collection()
        refs_col = referrals_collection()
        ...
        if not user:
            ...
            # Retornar estructura básica para usuario no encontrado
            return {
                "user_id": user_id,
        ...
        # Obtener últimos 5 referidos (solo si hay referidos)
        recent_referrals = []
        if total_referred > 0:
        ...
        for ref in recent_referrals:
            ...
            recent_list.append({
                ...
                "date": ref["activated_at"].strftime("%d/%m/%Y") if ref.get("activated_at") else "N/A"
            })
        ...
        # En caso de error crítico, retornar None para que el handler lo maneje
        return None
