...
EXCHANGES = ["Binance", "CoinEx", "KuCoin", "OKX"]  # Se pueden añadir más
...
async def handle_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if action == "register_exchange":
            await handle_register_exchange(query, user)
            return
...
# ======================================================
# HANDLER REFERRALS (ACTUALIZADO CON USDT)
# ======================================================
...
# ======================================================
# NUEVO HANDLER PARA REGISTRO DE EXCHANGE
# ======================================================

async def handle_register_exchange(query, user):
    try:
        keyboard = [
            [InlineKeyboardButton(name, callback_data=f"set_exchange_{name}")]
            for name in EXCHANGES
        ]
        keyboard.append([InlineKeyboardButton("⬅️ Volver al menú", callback_data="back_menu")])
        ...
        logger.error(f"Error en handle_register_exchange: {e}", exc_info=True)
        await query.edit_message_text(
            "❌ Error al cargar opciones de exchange.",
            reply_markup=back_to_menu(),
        )


async def handle_set_exchange(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    user_id = query.from_user.id
    users_col = users_collection()
    ...
    if not user:
        await query.edit_message_text("❌ Usuario no encontrado.", reply_markup=back_to_menu())
        return

    data = query.data
    if data.startswith("set_exchange_"):
        exchange = data.replace("set_exchange_", "")
        users_col.update_one({"user_id": user_id}, {"$set": {"exchange": exchange}})
        await query.edit_message_text(f"✅ Exchange registrado: {exchange}", reply_markup=back_to_menu())
...
async def handle_view_signals(query, user, admin, users_col):
    try:
        ...
        # Obtener señales según el plan real
        ...
        # ======================================================
        # MAPEAR SEÑAL AL EXCHANGE DEL USUARIO
        # ======================================================
        user_exchange = user.get("exchange", "Binance")
        user_signal = map_signal_to_exchange(user_signal, user_exchange)
...
# ======================================================
# UTILIDAD: MAPEO DE SEÑAL SEGÚN EXCHANGE
# ======================================================

def map_signal_to_exchange(signal, exchange):
    mapping = {
        "Binance": {},
        "CoinEx": {"BTCUSDT": "BTC/USDT"},  # Ejemplo
        "KuCoin": {"BTCUSDT": "BTC-USDT"},
        "OKX": {"BTCUSDT": "BTC-USDT"},
    }
    for key, val in mapping.get(exchange, {}).items():
        signal["pair"] = signal["pair"].replace(key, val)
        # Aquí se pueden mapear SL, TP si usan símbolos
    return signal
...
def get_handlers():
    return [
        CallbackQueryHandler(handle_menu, pattern="^(view_signals|plans|my_account|referrals|support|admin_panel|admin_activate_plan|register_exchange|back_menu)$"),
        CallbackQueryHandler(handle_copy_ref_code, pattern="^copy_ref_code$"),
        CallbackQueryHandler(handle_set_exchange, pattern="^set_exchange_"),
